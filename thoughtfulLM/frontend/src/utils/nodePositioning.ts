...
/**
 * Creates a bounded area to the left of a given node
 */
export const createBoundsLeftOfNode = (node: ReactFlowNode, showVisually = true): Bounds => {
  if (!node) {
    const defaultBounds = useBoundsStore.getState().defaultBounds;
    return defaultBounds;
  }
  ...
  // Width is 80% of the node width (matching right bounds)
  const boundsWidth = nodeWidth * 0.8;
  const boundsHeight = nodeHeight;
  // Place bounds to the left of the node with 10px spacing
...
/**
 * Creates a bounded area below a given node
 */
export const createBoundsBelowNode = (node: ReactFlowNode, showVisually = true): Bounds => {
  if (!node) {
    const defaultBounds = useBoundsStore.getState().defaultBounds;
    return defaultBounds;
  }
  ...
  // Width is 120% of the node width (matching above bounds)
  const boundsWidth = nodeWidth * 1.2;
  // Height is 70% of the thought node height (matching above bounds)
  const boundsHeight = nodeHeight * 0.7;
  // X offset to center the bounds below the node (accounts for the extra width)
  const xOffset = (boundsWidth - nodeWidth) / 2;
  // Y is 10px below the input node lower bound
